          ...
          {slotLabels.map((label, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider w-24 shrink-0 leading-tight">
                  {label}
                </span>

                {asset ? (
                  <div className="relative group flex-1 h-14 rounded-md overflow-hidden border border-gray-200">
                    <img
                      src={asset.url}
                      alt={asset.name}
                  ...
                  <>
                    <button
                      ...
                      className="flex-1 h-14 border-2 border-dashed border-gray-300 rounded-md text-[11px] text-gray-400 hover:border-gray-400 hover:text-gray-500 transition-colors flex items-center justify-center gap-1.5"
                    >
  ...
  return (
    <div>
      <div
        ...
        className="border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors duration-150 border-gray-300 hover:border-gray-400 bg-white"
      >
        ...
        <p className="text-xs text-gray-500">
          Drop images or <span className="text-blue-500 font-medium">browse</span>
        </p>
        <input
          ref={genericInputRef}
          type="file"
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          {assets.map((asset) => (
            <div key={asset.id} className="relative group">
              <img src={asset.url} alt={asset.name} className="w-full h-16 object-cover rounded-md border border-gray-200" />
              <button
                onClick={() => onRemove(asset.id)}
              ...
              <p className="text-[9px] text-gray-400 truncate mt-0.5">{asset.name}</p>
            </div>
          ))}
        </div>
      )}
