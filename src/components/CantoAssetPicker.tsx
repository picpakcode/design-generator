  ...
  if (value) {
    return (
      <div className="flex items-center gap-2">
        <div className="w-12 h-9 rounded-lg border border-gray-200 bg-gray-50 overflow-hidden shrink-0 flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] text-gray-600 truncate font-medium">{value.name}</p>
          <p className="text-[9px] text-gray-400">From Canto</p>
        </div>
        <button onClick={() => onChange(null)} className="text-[10px] text-gray-400 hover:text-red-500 transition-colors shrink-0">×</button>
      </div>
    )
  }
      ...
      {open && albumId && (
        <div>
          {loading ? (
            ...
            <p className="text-[10px] text-gray-400 text-center py-2">No assets found in this folder</p>
          ) : (
            <div className="flex gap-1.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
              {assets.map(a => (
                <button key={a.id} onClick={() => { onChange(a); setOpen(false) }}
                  className="shrink-0 w-[52px] h-[52px] rounded-lg border-2 border-transparent hover:border-gray-400 overflow-hidden transition-colors bg-gray-100 flex items-center justify-center"
                  title={a.name}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
