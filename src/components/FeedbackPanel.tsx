...
function Avatar({ name, isOwner }: { name: string; isOwner?: boolean }) {
  ...
  return (
    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
      isOwner ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600'
    }`}>
      {initials}
    </div>
  )
    ...
    <div className="flex items-center gap-1 mt-1.5 flex-wrap">
      {Object.entries(reactions).map(([emoji, names]) => (
        <button
          ...
          className="flex items-center gap-0.5 h-5 px-1.5 rounded-full bg-gray-100 hover:bg-indigo-50 hover:ring-1 hover:ring-indigo-200 transition-all text-[11px]"
          title={names.join(', ')}
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              ...
              {root.author_type === 'owner' && (
                <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-full">Owner</span>
              )}
              <span className="text-[10px] text-gray-300 ml-auto shrink-0">{timeAgo(root.created_at)}</span>
            </div>
          ...
          <button
            ...
            className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-all mt-0.5 ${
              isResolved
                ...
                : 'border border-gray-200 text-gray-300 hover:border-emerald-400 hover:text-emerald-500'
            }`}
          >
          ...
          {replies.map(reply => (
            <div key={reply.id} className="px-3 py-2 pl-8">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-[11px] font-semibold text-gray-800 truncate">{reply.author_name}</span>
                    {reply.author_type === 'owner' && (
                      <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-full">Owner</span>
                    )}
                    <span className="text-[10px] text-gray-300 ml-auto shrink-0">{timeAgo(reply.created_at)}</span>
                  </div>
...
export default function FeedbackPanel({ projectId, user, isOpen, onClose, onUnreadCount }: Props) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className={`fixed right-0 top-0 bottom-0 z-40 w-[340px] bg-white border-l border-gray-100 shadow-xl flex flex-col transition-transform duration-300 ease-out ${
        isOpen ? 'translate-x-0' : 'translate-x-full'
      }`}>

        {/* Header */}
        <div className="shrink-0 flex items-center gap-2.5 px-4 py-3.5 border-b border-gray-100 bg-white">
          ...
          <span className="font-semibold text-sm text-gray-900 flex-1">Feedback</span>
          {loading && (
          ...
          <button onClick={load} title="Refresh" className="w-6 h-6 flex items-center justify-center rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
          </button>
          <button onClick={onClose} className="w-6 h-6 flex items-center justify-center rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
        ...
        {/* Filter tabs */}
        <div className="shrink-0 flex gap-1 px-4 py-2 border-b border-gray-50">
          ...
          {!loading && filteredGroups.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-300 mt-0.5">Share the review link to get comments</p>
              </div>
            </div>
          ) : (
        ...
        <div className="shrink-0 px-4 py-2.5 border-t border-gray-50">
          <p className="text-[10px] text-gray-400 truncate">
            Replying as <span className="font-semibold text-gray-600">{user.email}</span>
          </p>
        </div>
      </div>
    </>
