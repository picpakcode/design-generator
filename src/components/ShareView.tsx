          ...
          {blocks.map(block => {
            ...
            return (
              <button
                ...
                title={block.label}
                className={`relative shrink-0 h-7 px-2.5 rounded-none text-[10px] font-semibold transition-all whitespace-nowrap ${
              ...
              >
                ...
                {(blockCommentCount > 0 || blockStatus) && (
                  <span className={`absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full border border-white ${
                    ...
                    blockStatus === 'approved'          ? 'bg-emerald-400' : 'bg-accent-400'
                  }`} />
                )}
              </button>
            )
                ...
                <div key={root.id} className={`rounded-none border transition-all ${isResolved ? 'border-gray-100 bg-gray-50/50 opacity-60' : `border-gray-100 bg-white${highlightThreads ? ' ring-1 ring-accent-200' : ''}`}`}>
                  ...
                  <div className="p-3">
                    <div className="flex items-start gap-2">
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5 ${root.author_type === 'owner' ? 'bg-accent-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          {root.author_type === 'owner' && <span className="text-[9px] font-bold text-accent-600 bg-accent-50 px-1.5 py-0.5 rounded-full shrink-0">Owner</span>}
                        ...
                        <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                          {Object.entries(root.reactions ?? {}).map(([emoji, names]) => (
                            <button key={emoji} onClick={() => handleReaction(root.id, emoji)}
                              title={names.join(', ')}
                              className="flex items-center gap-0.5 h-5 px-1.5 rounded-full bg-gray-100 hover:bg-accent-50 hover:ring-1 hover:ring-accent-200 transition-all text-[10px]">
                    ...
                    <div className="border-t border-gray-50 divide-y divide-gray-50">
                      {threadReplies.map(reply => (
                        <div key={reply.id} className="px-3 py-2 pl-7">
                          <div className="flex items-start gap-2">
                            <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold shrink-0 mt-0.5 ${reply.author_type === 'owner' ? 'bg-accent-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                              {reply.author_name[0]?.toUpperCase() ?? '?'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                <span className="text-[10px] font-semibold text-gray-800 truncate">{reply.author_name}</span>
                                {reply.author_type === 'owner' && <span className="text-[8px] font-bold text-accent-600 bg-accent-50 px-1 py-0.5 rounded-full shrink-0">Owner</span>}
                    ...
                    <div className="px-3 pb-2.5">
                      {isReplying ? (
                        ...
                        <button onClick={() => setReplyingTo(root.id)}
                          className="mt-1 text-[10px] text-gray-400 hover:text-accent-600 transition-colors font-medium">
                          Reply
                        </button>
                      )}
                    </div>
    ...
    return (
      <div className="flex flex-col h-screen overflow-hidden bg-white">
        ...
        <header className="shrink-0 bg-white border-b border-gray-100 px-5 py-3 flex items-center gap-3 shadow-sm z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-none font-medium ${
              data.accessLevel === 'edit' ? 'bg-accent-50 text-accent-600' : 'bg-gray-100 text-gray-500'
            }`}>
        ...
        {(() => {
          ...
          const current = tState.products[pidx]
          return (
            <div className="shrink-0 flex items-center gap-2 px-4 h-10 border-b border-gray-100 bg-white">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest shrink-0">Product</span>
              <span className="text-[10px] text-gray-300 tabular-nums shrink-0">{pidx + 1} / {tState.products.length}</span>

              {/* Dropdown selector */}
              <div className="relative flex-1 min-w-0">
                <button
                  onClick={() => setProductDropdownOpen(o => !o)}
                  className="w-full h-7 px-2.5 border border-gray-200 bg-white text-left text-[12px] text-gray-800 font-medium flex items-center gap-1.5 hover:border-gray-300 transition-colors rounded-none"
                >
                  <span className="flex-1 truncate">
                    {current?.productName || current?.sku || `Product ${pidx + 1}`}
                  </span>
                  <svg
                    className="w-3 h-3 text-gray-400 shrink-0 transition-transform"
                    style={{ transform: productDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}
                ...
                {productDropdownOpen && (
                  <>
                    ...
                    <div className="absolute top-[calc(100%+2px)] left-0 right-0 z-40 bg-white border border-gray-200 shadow-lg ring-1 ring-black/5 max-h-72 overflow-y-auto rounded-none">
                      {tState.products.map((p, i) => (
                        <button
                          key={p.id}
                          onClick={() => { setSelectedProductId(p.id); setSelectedBlockId(null); setProductDropdownOpen(false) }}
                          className={`w-full px-3 py-2 text-left text-[11px] flex items-baseline gap-2.5 transition-colors border-b border-gray-50 last:border-0 ${
                            p.id === productId ? 'bg-gray-50' : 'hover:bg-gray-50'
                          }`}
                        >
                          <span className="text-[10px] text-gray-400 font-medium shrink-0 tabular-nums w-4 text-right">{i + 1}</span>
                          <span className={`flex-1 leading-snug ${p.id === productId ? 'font-semibold text-gray-900' : 'text-gray-600'}`}>
                            {p.productName || p.sku || `Product ${i + 1}`}
                          </span>
                          {p.id === productId && (
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                <button
                  ...
                  disabled={!prev}
                  className="w-7 h-7 flex items-center justify-center rounded-none border border-gray-200 text-gray-400 hover:text-gray-700 hover:border-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
